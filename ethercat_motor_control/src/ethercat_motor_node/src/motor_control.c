...
typedef struct
{
    ...
    float spd_min, spd_max;
    float t_min, t_max;
    float i_min, i_max;
} MotorParams;

/** 型号 EC-A10020-P1-12 参数（ID: 1, 2, 4, 7, 8, 10） */
static const MotorParams PARAMS_A10020 = {
    .kp_min = 0.0f,   .kp_max = 500.0f,
    .kd_min = 0.0f,   .kd_max = 50.0f,
    .pos_min = -12.5f, .pos_max = 12.5f,
    .spd_min = -18.0f, .spd_max = 18.0f,
    .t_min = -150.0f,  .t_max = 150.0f,
    .i_min = -70.0f,   .i_max = 70.0f,
};

/** 型号 EC-A8112-P1-18 参数（ID: 3, 9, 13） */
static const MotorParams PARAMS_A8112 = {
    .kp_min = 0.0f,   .kp_max = 500.0f,
    .kd_min = 0.0f,   .kd_max = 5.0f,
    .pos_min = -12.5f, .pos_max = 12.5f,
    .spd_min = -18.0f, .spd_max = 18.0f,
    .t_min = -90.0f,   .t_max = 90.0f,
    .i_min = -60.0f,   .i_max = 60.0f,
};

/** 型号 EC-A4310-P2-36 参数（ID: 5, 6, 11, 12） */
static const MotorParams PARAMS_A4310 = {
    .kp_min = 0.0f,   .kp_max = 500.0f,
    .kd_min = 0.0f,   .kd_max = 5.0f,
    .pos_min = -12.5f, .pos_max = 12.5f,
    .spd_min = -18.0f, .spd_max = 18.0f,
    .t_min = -30.0f,   .t_max = 30.0f,
    .i_min = -30.0f,   .i_max = 30.0f,
};

/** 型号 EC-A6408-P2-25 参数（ID: 14, 15） */
static const MotorParams PARAMS_A6408 = {
    .kp_min = 0.0f,   .kp_max = 500.0f,
    .kd_min = 0.0f,   .kd_max = 5.0f,
    .pos_min = -12.5f, .pos_max = 12.5f,
    .spd_min = -18.0f, .spd_max = 18.0f,
    .t_min = -60.0f,   .t_max = 60.0f,
    .i_min = -60.0f,   .i_max = 60.0f,
};

/** 无效 ID 使用的默认参数（全 0） */
static const MotorParams PARAMS_ZERO = {
    .kp_min = 0.0f,   .kp_max = 0.0f,
    .kd_min = 0.0f,   .kd_max = 0.0f,
    .pos_min = 0.0f,  .pos_max = 0.0f,
    .spd_min = 0.0f,  .spd_max = 0.0f,
    .t_min = 0.0f,    .t_max = 0.0f,
    .i_min = 0.0f,    .i_max = 0.0f,
};

/** 根据电机 ID 返回对应型号参数；未在 1–15 范围内的 ID 返回全 0 参数 */
static const MotorParams* get_motor_params(uint16_t motor_id)
{
    switch (motor_id)
    {
    case 1: case 2: case 4: case 7: case 8: case 10:
        return &PARAMS_A10020;
                         ...
                         float spd, float tor)
{
    ...
    if (pos > p->pos_max)
        pos = p->pos_max;
    else if (pos < p->pos_min)
        pos = p->pos_min;
    if (spd > p->spd_max)
        spd = p->spd_max;
    else if (spd < p->spd_min)
        spd = p->spd_min;
    if (tor > p->t_max)
        tor = p->t_max;
    else if (tor < p->t_min)
    ...
    kp_int = float_to_uint(kp, p->kp_min, p->kp_max, 12);
    kd_int = float_to_uint(kd, p->kd_min, p->kd_max, 9);
    pos_int = float_to_uint(pos, p->pos_min, p->pos_max, 16);
    spd_int = float_to_uint(spd, p->spd_min, p->spd_max, 12);
    tor_int = float_to_uint(tor, p->t_min, p->t_max, 12);
...
static void handle_response_mode(Motor_Msg* motor_data, OD_Motor_Msg* motor_msg, int ack_status, int print)
{
    ...
    if (ack_status == 1) // response frame 1
    {
        ...
        const MotorParams* p = get_motor_params(motor_id_check);

        motor_msg->angle_actual_rad = uint_to_float(pos_int, p->pos_min, p->pos_max, 16);
        motor_msg->speed_actual_rad = uint_to_float(spd_int, p->spd_min, p->spd_max, 12);
        motor_msg->current_actual_float = uint_to_float(cur_int, p->i_min, p->i_max, 12);
    ...
    else if (ack_status == 2) // response frame 2
    {
        ...
        motor_msg->angle_actual_rad = motor_msg->angle_actual_float;  /* 供 /low_state q 使用 */
    ...
    else if (ack_status == 3) // response frame 3
    {
        ...
        motor_msg->speed_actual_rad = motor_msg->speed_actual_float;   /* 供 /low_state dq 使用 */
        motor_msg->current_actual_int = motor_data->data[5] << 8 | motor_data->data[6];
        motor_msg->temperature = (motor_data->data[7] - 50) / 2;
        motor_msg->current_actual_float = motor_msg->current_actual_int / 100.0f;
    }
    ...
    else if (ack_status == 5) // response frame 5
    {
        ...
        if (motor_data->dlc == 6)
        {
            ...
            *table[motor_comm_fbd.INS_code - 1] = rv_type_convert.to_float;
            /* 同步 _rad 供 /low_state 使用 */
            if (motor_comm_fbd.INS_code == 1) motor_msg->angle_actual_rad = motor_msg->angle_actual_float;
            if (motor_comm_fbd.INS_code == 2) motor_msg->speed_actual_rad = motor_msg->speed_actual_float;
        }
